...
- **Session C — 60 min, weekend.** 45 min **interleaved mixed quiz** — problems from every topic so far, labels stripped, weighted toward older material (the operative parameters — count, mix, weighting window — live in [[Interleaver — Weekly Quiz Prompt]]). Interleaving does my spacing at the problem level, on top of (not instead of) SRS. **Division of labor (decided 2026-08-14):** this weekly set is the home of **multi-step transfer problems**; atomic "which one *here*?" decisions belong to the daily [[Quizmaster Design Spec|Quizmaster]] quiz, whose state machine gives them true spacing. The two stay deliberately separate — each resurfaces its own misses; the domain Mistake Logs are the shared ledger. 15 min **delta pass**: the last month of `?` and `BUG` marks plus everything I missed this week, spoken once.
- **Alongside:** the daily **SRS habit** (protected — cheap, existing, mine), the daily **[[Quizmaster Design Spec|Quizmaster]] quiz** (~10 min: answer closed-book, then the one-command grade — machinery and knobs in the spec), weekly **SRS pruning** (delete cards I can't justify; cap daily reviews), and **ship or advance a real project** — the project is the point; the notes are fuel.

**Degraded mode — when the week goes wrong, cut in this order:** the daily Quizmaster quiz → delta pass → the 10-min defend → half the weekly quiz. **Never cut the brain dump or the spoken exam** — those 35 minutes carry most of the effect. A bad week is 35 minutes, not zero. (The daily quiz goes first on purpose: the three sessions are the spine; the extra layer is the first thing adherence is allowed to eat.)
